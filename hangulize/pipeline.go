...
//
// Meaningful letter is the letter which appears in the quantize/transcribe
...
// 3. Quantize (Subwords -> Subwords[level=1])
//
...
// English or Franch.
//
func (p *pipeline) quantize(subwords []subword) []subword {
	...
	for i, sw := range subwords {
		...
		for j, rule := range p.h.spec.Quantize {
...
func (p *pipeline) transcribe(subwords []subword) []subword {
	...
	for i, sw := range subwords {
		...
		// transcribe is not quantize.  A result of a replacement is not the
	...
	// Discard level=1 subwords.  They have been generated by "3. Quantize" but
