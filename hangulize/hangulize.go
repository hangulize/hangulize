...
pipeline has 5 steps: "Normalize", "Group", "Quantize", "Transcribe", and
...
quantize/transcribe rules.  The "3. Quantize" step minimizes the gap between
...
We will write many patterns in quantize/transcribe rules soon.  Some
expressions may appear many times annoyingly.  To not repeat ourselves, we can
...
Now we can write "quantize" rules.  There are Pattern and RPattern.  Pattern
...
"transcribe" rules are exactly same with "quantize" rules.  But it's RPatterns
represent Hangul Jamo phonemes.  In contrast to "quantize", a replaced word
